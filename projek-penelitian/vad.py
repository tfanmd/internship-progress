...
def rekam_dengan_vad(file_output="input_live.wav"):
    r = sr.Recognizer()
    r.pause_threshold = 2.0 # Nunggu lu diam 2 detik penuh baru berhenti
    r.non_speaking_duration = 0.5

    with sr.Microphone(sample_rate=16000) as source:
        print("\n" + "="*40)
        print("🎛️ Menyesuaikan VAD dengan kebisingan ruangan... (Diam bentar ya)")
        r.adjust_for_ambient_noise(source, duration=2.0) # Kalibrasi mic otomatis
        
        print("\n🟢 SILAKAN NGOMONG! (Sistem otomatis berhenti merekam kalau lu diam)")
        
        try:
            # Sistem akan mendengarkan. timeout=Batas nunggu suara, phrase_time_limit=Batas max ngomong
            audio = r.listen(source, timeout=10, phrase_time_limit=15)
            
            # Simpan suara yang berhasil ditangkap VAD ke file .wav
            with open(file_output, "wb") as f:
                f.write(audio.get_wav_data())
                
            print("✅ Perekaman selesai! (Keheningan terdeteksi)")
            
        except sr.WaitTimeoutError:
            print("❌ Lu diam terlalu lama, perekaman dibatalkan.")
            return None
...
def smart_headset(file_audio_masuk):
    ...
    end_mt = time.time() #  Stop Stopwatch MT
...
if __name__ == "__main__":
    print("🚀 MEMULAI SISTEM SMART HEADSET (VAD MODE)...")
    
    # 1. Panggil fungsi rekam VAD yang otomatis
    file_suara_langsung = rekam_dengan_vad()
    
    # 2. Kalau ada suara yang terekam, langsung proses ke AI
    if file_suara_langsung:
